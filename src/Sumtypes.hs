...
replaceGenericTypesOnCases mappings cases =
  ...
  where replaceOnCase theCase =
          let newTys = map (replaceTyVars UnificationIgnoresLifetimes mappings) (caseTys theCase)
        ...
        template =
          ...
          (\(FuncTy _ concreteStructTy) ->
             let mappings = unifySignatures structTy concreteStructTy
                 correctedTys = map (replaceTyVars UnificationIgnoresLifetimes mappings) (caseTys sumtypeCase)
...
concreteStr typeEnv env insidePath concreteStructTy@(StructTy typeName _) cases strOrPrn =
  instanceBinder (SymPath insidePath strOrPrn) (FuncTy [RefTy concreteStructTy (Just (VarTy "q"))] StringTy) template doc
...
concreteSumtypeCopy insidePath typeEnv env structTy@(StructTy typeName _) cases =
  instanceBinder (SymPath insidePath "copy") (FuncTy [RefTy structTy (Just (VarTy "q"))] structTy) template doc
